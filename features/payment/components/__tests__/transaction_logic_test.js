...
function runTests() {
    ...
    assert('Real Sample: DEBIT "300"', formatTransactionAmount('DEBIT', '300'), '-₹300');
    assert('Real Sample: CREDIT "1000"', formatTransactionAmount('CREDIT', '1000'), '+₹1,000');
    ...
    assert('Status Color: COMPLETED', getStatusColor('COMPLETED'), colors.success);
    ...
    if (formattedDate.includes('2026') && formattedDate.includes('Feb')) {
        ...
        console.log(`[FAIL] Date Formatting: Expected Feb/2026. Got: ${formattedDate}`);
        failed++;
    }
